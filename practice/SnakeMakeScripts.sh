...
12 Error logs
.snakemake/slurm_logs/rule_variantfiltering/3852849.log 
.snakemake/log/2026-04-14T111137.364958.snakemake.log
...
rule gzip_vcf:
    input:
        "results/vcf/Diglossa_glauca_multisample.vcf"
    output:
        "results/vcf/Diglossa_glauca_multisample.vcf.gz"
    shell:
        """
        gzip -c {input} > {output}
        """
...
rule variantfiltering:
    ...
    shell:
        """
        vcftools --gzvcf {input} --remove-indels --minQ 40 --thin 50 --min-meanDP 6 --recode --stdout | bgzip -c {output} #less strict cutoffs given it's only two samples I am running
